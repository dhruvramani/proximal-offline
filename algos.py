...
class ClonedPolicy(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, num_samples_match=10):
        for it in range(iterations):
            ...
            recon, vmean, vstd = self.vae(state, action)
            recon_loss = F.mse_loss(recon, action)
            KL_loss = -0.5 * (1 + torch.log(vstd.pow(2)) - vmean.pow(2) - vstd.pow(2)).mean()
        ...
        for it in range(iterations):
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                for i in range(self.train_pi_iters):
                    ...
                    ratio = torch.exp((logp_actor - logp_cloned).clamp(max=1e4)) #p_actor / (p_cloned + 1e-6) #
                    ratio[ratio == float("Inf")] = 1000.0
