...
class MLPCritic(nn.Module):
    ...
    def forward(self, obs, action):
        return torch.squeeze(self.q_net(torch.cat((obs, action), -1)), -1) # Critical to ensure v has right shape.
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, typeA='cont',
                 hidden_sizes=(400, 300), activation=nn.Tanh):
        super().__init__()
